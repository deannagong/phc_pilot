...
latent <- list(
  WorkingEnvironment = c("Responsibility_Clarity", "Workload_Reasonable", 
                          "Authority_Understanding", "Opinion_Expression"),
  Fatigue = c("fatigue_01", "fatigue_02", "fatigue_03", "fatigue_04", "fatigue_05"),
  PHCtask = c("Health_Center_2", "Health_Center_3", "Health_Center_4", 
               "Health_Center_5", "Health_Center_6", "Health_Center_7"),
  Community = c("Understand_CommunityNeeds", "CareSystem", 
                "TailoredServices", "ResourceIntegration")
)

# Define regressions
regression <- list(
  Fatigue = "WorkingEnvironment",
  PHCtask = c("WorkingEnvironment","Fatigue"),
  TransformToHealthCenter = c("PHCtask", "Community")
)
...
# Map mediator -> outcome 
mediation <- list(
  PHCtask = "Fatigue",   # PHCtask is mediated by Fatigue (IV is WorkingEnvironment)
  Fatigue = "WorkingEnvironment"
)

# Define indirect query for convenience
IV <- "WorkingEnvironment"; M <- "Fatigue"; DV <- "PHCtask"
...
# Fit
fit.sem <- sem(model, data = df, std.lv = TRUE, estimator = "WLSMV", ordered = TRUE)
